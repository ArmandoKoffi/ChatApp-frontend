...
export function GroupChatArea({
  ...
  const handleToggleBlock = (userId: string, block: boolean) => {
    ...
    console.log(
      `${block ? t("blocked") : t("unblocked")} utilisateur:`,
      userId
    );
  };
  ...
  const addFileMessage = (file: File) => {
    const newMessage = {
      ...
      fileSize: !file.type.startsWith("image/")
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-6 bg-gray-50 animate-fade-in">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600">{t("selectRoomDesc")}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center">
          ...
          <div className="animate-fade-in">
            ...
            <p className="text-sm text-gray-500">
              {currentRoom.connectedUsers} {t("online")}
            </p>
          </div>
        </div>
          ...
          >
            <div className="flex items-end space-x-2 max-w-xs lg:max-w-md">
              {msg.sender === "other" && (
                <div className="relative group">
                  ...
                  <div className="absolute bottom-full left-0 mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-xs rounded px-2 py-1 whitespace-nowrap animate-fade-in">
                    <button
                      onClick={() =>
                        handleUserBlock(msg.senderId, msg.senderName)
                      }
              ...
              <div className="space-y-1">
                {msg.sender === "other" && (
                  <p className="text-xs text-gray-500 ml-2 animate-fade-in">
                    {msg.senderName}
                  </p>
                )}
                <div
